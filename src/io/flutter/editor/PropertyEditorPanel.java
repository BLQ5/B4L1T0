...
public class PropertyEditorPanel extends JBPanel {
  ...
  public PropertyEditorPanel(FlutterApp app, Project project, DiagnosticsNode node, XSourcePosition p, FlutterDartAnalysisServer flutterDartAnalysisService) {
    super();
...
//    setBorder(JBUI.Borders.empty(5));
    ...
    if (node != null) {
      ...
      group.safeWhenComplete(node.getProperties(group), (diagnosticProperties, error) -> {
        if (error != null || diagnosticProperties == null) { return; }
          ...
          public void mouseClicked(MouseEvent e) {
            ...
            listeners.add(new ColorPickerListener() {
              ...
              public void colorChanged(Color color) {
                ...
                widgetColor = color;
                long value = ((long)color.getAlpha() << 24) | ((long)color.getRed() << 16) | (long)(color.getGreen() << 8) | (long)color.getBlue();
...
/*      add(new JColorChooser(), "span");
  ...
  public static Balloon showPopup(FlutterApp app, EditorEx editor, DiagnosticsNode node, XSourcePositionImpl position, FlutterDartAnalysisServer service, Point point) {
    PropertyEditorPanel panel =
      new PropertyEditorPanel(app, editor.getProject(), node, position, service);
    panel.setBackground(IdeTooltipManager.GRAPHITE_COLOR);
...
//    balloon.setBounds();
    // XXX provide another option if TextRange is null.
    if (position != null) {
      TextRange textRange = new TextRange(position.getOffset(), position.getOffset() + 1);
